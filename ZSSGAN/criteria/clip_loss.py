...
# import ZSSGAN.criteria.net as net
# from ZSSGAN.criteria.function import calc_mean_std

# from ZSSGAN.criteria.region_loss.model import BiSeNet

from utils.text_templates import imagenet_templates, part_templates, imagenet_templates_small
import criteria.net as net
from criteria.function import calc_mean_std

from criteria.region_loss.model import BiSeNet
# from ZSSGAN.criteria.id_loss.id_loss import IDLoss
from criteria.id_loss.id_loss import IDLoss
...
class CLIPLoss(torch.nn.Module):
    def __init__(self, device, lambda_direction=1., lambda_patch=0., lambda_global=0., lambda_manifold=0., lambda_texture=0., patch_loss_type='mae', direction_loss_type='cosine', clip_model='ViT-B/32'):
        ...
        self.preresize = transforms.Compose(
                    [transforms.ToTensor(), transforms.Resize((1024, 1024), interpolation=0)])
        ...
        self.model_cnn, preprocess_cnn = clip.load("/home/chenzhuo/workspace/3DAnimationGAN/model_zoo/RN50.pt", device=self.device)
        ...
        self.id_net = IDLoss(self.id_ckpt_path).to(self.device)

        # VGG encoder
        
        vgg = net.vgg.to(self.device)
        vgg.load_state_dict(torch.load("//home/chenzhuo/workspace/3DAnimationGAN/model_zoo/vgg_normalised.pth"))
    ...
    def compute_source_feature(self, source_images: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():

            src_encoding = self.get_image_features(source_images)
            source_feature = src_encoding.mean(dim=0, keepdim=True)
    ...
    def compute_2styleimg_direction(self, target_images: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():

            # mean direction -> each target image represents a specific direction in a batch. 
            target_encoding = self.get_image_features(target_images)
            target_encoding = target_encoding.mean(dim=0, keepdim=True)

            direction = target_encoding - self.source_feature
            direction /= direction.norm(dim=-1, keepdim=True)

        return direction

    def compute_imglist2imglist_direction(self, source_images: list, target_images: list) -> torch.Tensor:
        with torch.no_grad():

            src_encodings = []
            for source_img in source_images:
                source_preprocessed = self.clip_preprocess(Image.open(source_img)).unsqueeze(0).to(self.device)
                
                source_encoding = self.model.encode_image(source_preprocessed)
                source_encoding /= source_encoding.norm(dim=-1, keepdim=True)

                src_encodings.append(source_encoding)
            
            src_encoding = torch.cat(src_encodings, axis=0)
            src_encoding = src_encoding.mean(dim=0, keepdim=True)
    ...
    def clip_directional_loss_with_fixed_direction(self, src_img: torch.Tensor, target_img: torch.Tensor, target_direction: torch.Tensor) -> torch.Tensor:

        # target_direction  /= target_direction.norm(dim=-1, keepdim=True)
    ...
    # text2text_distance
    def get_text_distance(self, source_text:str, target_text:str) -> torch.Tensor:
        source_features = self.get_text_features(source_text)
        target_features = self.get_text_features(target_text)

        return self.l2_loss(source_features, target_features)

    def compute_region_distance(self, source_text:str):
        region_distance = {}
        for key, value in self.atts.items():
            text_distance = self.get_text_distance(source_text, value)
            region_distance[value] = text_distance
    ...
    def local_clip_loss(self, source_img: torch.Tensor, target_img: torch.Tensor):

        source_encoding = self.get_image_features(source_img)

        # target_preprocessed = self.clip_preprocess(target_img)
        target_encoding = self.get_image_features(target_img)

        return self.mse_loss(source_encoding, target_encoding)
    ...
    def region_mask(self, im, parsing_anno, stride, index=[17], reverse_mask=False):
        ...
        # print("vis_parsing_anno", vis_parsing_anno.shape)
        # vis_parsing_anno = cv2.resize(vis_parsing_anno, None, fx=stride, fy=stride, interpolation=cv2.INTER_NEAREST)

        zeros = torch.zeros_like(vis_parsing_anno)
        ones = torch.ones_like(vis_parsing_anno)
        zeros_mask = torch.zeros_like(vis_parsing_anno)
        ones_mask = torch.ones_like(vis_parsing_anno)

        for index_i in index:
            if reverse_mask:
                hair_mask = torch.where(vis_parsing_anno == index_i, ones, zeros) # 17 -> hair
                zeros_mask += hair_mask
                mask = zeros_mask
            else:
                hair_mask = torch.where(vis_parsing_anno == index_i, zeros, ones) # 17 -> hair
                ones_mask *= hair_mask
                mask = ones_mask
            
        return mask

    def face_segmentation(self, frozen_img, trainable_img, index=[17], reverse_mask=False):
        ...
        t_out = self.parsing_net(t_img)[0]
        f_parsing = f_out.argmax(1)
        t_parsing = t_out.argmax(1)
        # print("f_parsing.shape", f_parsing.shape)
        # print(parsing)

        f_mask = self.region_mask(f_img, f_parsing, stride=1, index=index, reverse_mask=reverse_mask).unsqueeze(1).repeat(1, 3, 1, 1)
        t_mask = self.region_mask(t_img, t_parsing, stride=1, index=index, reverse_mask=reverse_mask).unsqueeze(1).repeat(1, 3, 1, 1)
        # print("f_mask.shape", f_mask.shape)
        # if mask != None:
        #     f_mask = torch.where(f_mask == 0, mask, f_mask)
        #     t_mask = torch.where(t_mask == 0, mask, t_mask)
    ...
    def region_loss(self, src_img: torch.Tensor, target_img: torch.Tensor, index=[17]) -> torch.Tensor:
        f_img, t_img = self.face_segmentation(src_img, target_img, index)

        return self.l2_loss(f_img, t_img)
    ...
    def vgg_feature_loss(self, src_img: torch.Tensor, target_img: torch.Tensor) -> torch.Tensor:
        style_feats = self.encode_with_intermediate(src_img)
        g_t_feats = self.encode_with_intermediate(target_img)
        # print(g_t_feats[0].size(), style_feats[0].size())
    ...
    def id_loss(self, src_img, target_img):
        id_difference = self.id_net(src_img, target_img)
        return id_difference

    def forward(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str, target_direction: torch.Tensor = None, texture_image: torch.Tensor = None, style_img: torch.Tensor = None, index=[3, 4]):
        clip_loss = 0.0
        ...
        if self.lambda_direction:
            if target_direction is None:
                ...
                clip_loss += self.lambda_direction * self.clip_directional_loss_with_fixed_direction(src_img, target_img, target_direction)
        ...
        if self.lambda_texture and (texture_image is not None):
            clip_loss += self.lambda_texture * self.vgg_feature_loss(texture_image, target_img)

        if self.lambda_region:
            region_loss = self.region_loss(src_img, target_img, index=[17])
            clip_loss += self.lambda_region * region_loss

        if self.lambda_local:
            local_clip_loss = self.lambda_local * self.local_clip_loss(target_img, style_img)
            print("local_clip_loss:", local_clip_loss)
            clip_loss += local_clip_loss

        if self.lambda_id:
            id_loss = self.id_loss(src_img, target_img)
            print("id_loss", id_loss)
            clip_loss += self.lambda_id * id_loss
