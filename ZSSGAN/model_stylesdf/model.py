...
from model_stylesdf.volume_renderer import VolumeFeatureRenderer
# from model_stylesdf.volume_renderer_inference import VolumeFeatureRenderer
from model_stylesdf.op import FusedLeakyReLU, fused_leaky_relu, upfirdn2d
...
from model_stylesdf.face_parsing.faceparsing import FaceParsing
...
class Decoder(nn.Module):
    def __init__(self, model_opt, blur_kernel=[1, 3, 3, 1]):
        ...
        self.size = model_opt.model_size
    ...
    def styles_and_noise_forward(self, styles, noise, inject_index=None, truncation=1,
        ...
        if len(styles) < 2:
            ...
            if single_layer:
                ...
                latent[:, inject_index, :] = styles[1].unsqueeze(1)
                # latent[:, inject_index, :] = latent2[:, inject_index, :]
...
class Generator(nn.Module):
    def __init__(self, model_opt, renderer_opt, blur_kernel=[1, 3, 3, 1], ema=False, full_pipeline=True):
        super().__init__()
        #parsing
        self.parsing = FaceParsing(device='cuda:0')
        self.size = model_opt.model_size
    ...
    def styles_and_noise_forward(self, styles, inject_index=None, truncation=1,
        ...
        # add style mixing in 3D architecture.
        if len(styles) < 2:
            return styles
        else:
            if inject_index is None:
    ...
    def forward(self, styles, cam_poses, focals, near=0.88, far=1.12, return_latents=False,
                ...
                project_noise=False, mesh_path=None, return_depth=False, return_normal=False, 
                clip_shape_latent=None, clip_style_latent=None, target_ldks=None):
        ...
        with torch.set_grad_enabled(self.is_train and self.train_renderer):
            ...
            thumb_rgb, features, sdf, mask, xyz, eikonal_term, depth, normal = self.renderer(cam_poses, focals, near, far, styles=latent[0], return_eikonal=return_eikonal, clip_shape_latent=clip_shape_latent, clip_style_latent=clip_style_latent, target_ldks=target_ldks)

        if self.full_pipeline:
            if latent[0].ndim == 3:
                latent = [latent[0][:, 0, :].squeeze(1)]
        ...
        if remove_bg:
            # with torch.no_grad():
            self.parsing.parsing_net.eval()
            hair_mask = self.parsing.face_mask(frozen_img=rgb, index=17, reverse_mask=True)

            img_size = rgb.shape[-2]
            upsample = transforms.Resize((img_size, img_size))
            depth_hd = upsample(depth)

            zeros = torch.zeros_like(depth_hd)
            ones = torch.ones_like(depth_hd)
            delta = 1e-5
            
            fg_mask = torch.where(depth_hd > (1.1-delta), zeros, ones)
            bg_mask = ones - fg_mask
            num_fg = torch.nonzero(fg_mask)

            real_hair_mask = hair_mask * fg_mask
            extra_hair = hair_mask * bg_mask
