...
class SirenGenerator(nn.Module):
    ...
    def forward(self, x, styles, sigmoid_beta=0, return_normal=False, clip_shape_latent=None, clip_style_latent=None):
        ...
        if self.enable_hypernetwork and clip_style_latent != None:
            ...
            # for i in range(8):
            #     print(i)
            #     params_delta_shape[i][0] = params_delta_shape[i][0] + params_delta_style[i][0]
            #     params_delta_shape[i][1] = params_delta_shape[i][1] + params_delta_style[i][1]
            
            mlp_out = self.pts_linears[0](mlp_out, styles, shape_ratio * params_delta_shape[0][0], shape_ratio * params_delta_shape[0][1])
            mlp_out = self.pts_linears[1](mlp_out, styles, shape_ratio * params_delta_shape[1][0], shape_ratio * params_delta_shape[1][1])
            mlp_out = self.pts_linears[2](mlp_out, styles, shape_ratio * params_delta_shape[2][0], shape_ratio * params_delta_style[2][1])
            mlp_out = self.pts_linears[3](mlp_out, styles, style_ratio * params_delta_style[3][0], style_ratio * params_delta_style[3][1])
            mlp_out = self.pts_linears[4](mlp_out, styles, shape_ratio * params_delta_style[4][0], style_ratio * params_delta_style[4][1])
            mlp_out = self.pts_linears[5](mlp_out, styles, shape_ratio * params_delta_style[5][0], style_ratio * params_delta_style[5][1])
            mlp_out = self.pts_linears[6](mlp_out, styles, shape_ratio * params_delta_style[6][0], style_ratio * params_delta_style[6][1])
            mlp_out = self.pts_linears[7](mlp_out, styles, shape_ratio * params_delta_style[7][0], style_ratio * params_delta_style[7][1])
        ...
        if not self.enable_hypernetwork or clip_style_latent==None:
            ...
            out_features = self.views_linears(mlp_out, styles, style_ratio * params_delta_style[8][0], style_ratio * params_delta_style[8][1])
            # out_features = self.views_linears(mlp_out, styles)
            rgb = self.rgb_linear(out_features)
