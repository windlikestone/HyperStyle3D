...
class SG2Generator(torch.nn.Module):
    ...
    def forward(self, sample_z, cam_extrinsics, focal, near, far, input_is_latent=False, clip_style_latent=None):
        # print(latent_z)
...
class ZSSGAN(torch.nn.Module):
    ...
    def fine_tune_geo(self):
        ...
        self.source_ldks = torch.from_numpy(np.loadtxt(self.initail_ldks_path)).to(self.device)
        # self.target_ldks = torch.from_numpy(np.loadtxt(self.target_ldks_path))

        initial_target_ldks = self.source_ldks

        target_ldks = torch.Tensor(initial_target_ldks.cpu().detach().numpy()).to(self.device)
    ...
    def forward(self,latent_z,cam_extrinsics,focal,near,far, target_class, style_img, input_is_latent=False):
        ...
        trainable_output = self.generator_trainable(latent_z, cam_extrinsics, focal, near, far, input_is_latent, clip_style_latent)
