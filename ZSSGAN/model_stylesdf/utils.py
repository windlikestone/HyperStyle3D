...
######################### xyz2mesh for experiments ##################
def xyz2mesh(xyz, background_weights=None, rgb=None,filename=None):
    b, _, h, w = xyz.shape
    x, y = np.meshgrid(np.arange(w), np.arange(h))

    # Extract meshes from xyz maps
    tri = Delaunay(np.concatenate((x.reshape((h*w, 1)), y.reshape((h*w, 1))), 1))
    faces = tri.simplices
    #invert normals
    faces[:,[0, 1]] = faces[:,[1, 0]]

    # generate UV map to the full resolution RGB image
    if rgb != None:
        u, v = np.meshgrid(np.arange(0.5/w,(w+0.5)/w,1/w), np.arange((h-0.5)/h,-0.5/h,-1/h)) # bottom left corner is (0,0) upper right corner is (1,1)
        uv_map = np.concatenate([np.expand_dims(u,-1), np.expand_dims(v,-1)], -1).reshape((h*w,2))

    if background_weights == None and rgb == None:
        mesh = trimesh.Trimesh(xyz.squeeze(0).permute(1,2,0).view(h*w,3).cpu().numpy(), faces)
    elif rgb != None: # add UV Map to the mesh
        np.savez(filename, xyz=xyz.squeeze(0).permute(1,2,0).numpy(),faces=faces,uv_map=uv_map, bg_opacity=background_weights.squeeze(0).permute(1,2,0).numpy())
        mesh = None
        # mesh = trimesh.Trimesh(xyz.squeeze(0).permute(1,2,0).view(h*w,3).cpu().numpy(), faces)
        # pil_image = Image.fromarray(np.uint8(127.5*(rgb.squeeze(0).permute(1,2,0).clamp(-1,1).numpy()+1)))
        # mesh.visual = trimesh.visual.TextureVisuals(uv=uv_map, image=pil_image)
    else:
        mesh = trimesh.Trimesh(xyz.squeeze(0).permute(1,2,0).view(h*w,3).numpy(), faces,
                               vertex_colors=background_weights.squeeze(0).repeat(3,1,1).permute(1,2,0).view(h*w,3).numpy())

    return mesh

######################## reprojection for color consistency ################
def project_color(source_rgb, source_depth, frontal_xyz, transform, focal, nbins=128, occ_thres=2):
    # move transform and focal to cpu
    transform, focal = transform.cpu(), focal.cpu()
    # resize input xyz map
    batch, _, height_d, width_d = source_depth.shape
    _, _, height, width = source_rgb.shape
    frontal_xyz = F.interpolate(frontal_xyz, size=(height, width), mode='bilinear')#.squeeze(1)
    source_depth = F.interpolate(source_depth, size=(height, width), mode='bilinear')

    # transform frontal_xyz to camera viewpoint
    # translate points to camera origin
    transformed_xyz = frontal_xyz - transform[:,:3,3,None,None]

    # Transpose R
    transform[:,:3,:3] = transform[:,:3,:3].permute(0,2,1).contiguous()

    # Rotate camera viewpoint
    transformed_xyz = torch.sum(transformed_xyz.permute(0,2,3,1).contiguous()[..., None, :] * transform[:,:3,:3][:,None,None,:3,:3], -1)
    transformed_xyz = transformed_xyz.permute(0,3,1,2).contiguous()

    # # continue transforming, camera's z axis points at the origin so need to flip y & z axes
    # transformed_xyz[:,1:,...] = -transformed_xyz[:,1:,...]

    # project 3d points to pixel locations in the initial viewpoint
    scale = height / height_d
    focal = focal.view(-1,1,1,1)

    pts_2d = torch.cat([(focal * scale * transformed_xyz[:,:1,...] + width * .5) / torch.abs(transformed_xyz[:,2:,...]),
                        (-focal * scale * transformed_xyz[:,1:2,...] + height * .5) / torch.abs(transformed_xyz[:,2:,...])], 1)

    # normalize sample points to [-1,1] and sample noise values from the intial viewpoint
    pts_2d[:,0,...] = 2 * pts_2d[:,0,...] / width - 1
    pts_2d[:,1,...] = 2 * pts_2d[:,1,...] / width - 1
    pts_2d = pts_2d.permute(0,2,3,1)
    transformed_rgb = F.grid_sample(source_rgb.expand(batch,-1,-1,-1), pts_2d, mode='bilinear')
    transformed_depth = F.grid_sample(source_depth.expand(batch,-1,-1,-1), pts_2d, mode='bilinear')

    # reject occulions larger then 2 bin sizes (camera z axis points at the viewer, so need to negate the z values)
    bin_size = 0.24 / nbins
    is_valid = torch.abs(transformed_depth - (-transformed_xyz[:,2:])) < occ_thres * bin_size
