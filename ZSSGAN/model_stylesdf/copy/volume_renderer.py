...
class SirenGenerator(nn.Module):
    def __init__(self, D=8, W=256, style_dim=256, input_ch=3, input_ch_views=3, output_ch=4,
        ...
        if self.enable_hypernetwork:
            ...
            # self.hyper_linears_shape = hyperlayers.HyperFC(in_ch_pos=input_ch,
    ...
    def forward(self, x, styles, sigmoid_beta=0, return_normal=False, clip_style_latent=None):
        ...
        if self.enable_hypernetwork and clip_style_latent != None:
            ...
            # params_delta_shape = self.hyper_linears_shape(clip_style_latent)
            params_delta = self.hyper_linears(clip_style_latent)
            # print("hyperlayers")
            
            mlp_out = self.pts_linears[0](mlp_out, styles)
            mlp_out = self.pts_linears[1](mlp_out, styles)
            mlp_out = self.pts_linears[2](mlp_out, styles)
            mlp_out = self.pts_linears[3](mlp_out, styles)
            mlp_out = self.pts_linears[4](mlp_out, styles)
            mlp_out = self.pts_linears[5](mlp_out, styles)
            mlp_out = self.pts_linears[6](mlp_out, styles)
            mlp_out = self.pts_linears[7](mlp_out, styles)
        ...
        if not self.enable_hypernetwork or clip_style_latent==None:
            ...
            # params_delta = self.hyper_linears(clip_style_latent)
            out_features = self.views_linears(mlp_out, styles, params_delta[0][0], params_delta[0][1])
            # out_features = self.views_linears(mlp_out, styles)
            rgb = self.rgb_linear(out_features, params_delta[1][0], params_delta[1][1])
...
class VolumeFeatureRenderer(nn.Module):
    ...
    def run_network(self, inputs, viewdirs, styles=None, clip_style_latent=None):
        ...
        outputs = self.network(net_inputs, styles=styles, sigmoid_beta=self.sigmoid_beta, return_normal=self.return_normal, clip_style_latent=clip_style_latent)

        return outputs

    def render_rays(self, ray_batch, styles=None, return_eikonal=False, clip_style_latent=None, target_ldks=None):
        ...
        raw = self.run_network(normalized_pts, viewdirs, styles=styles, clip_style_latent=clip_style_latent)
    ...
    def render(self, focal, c2w, near, far, styles, c2w_staticcam=None, return_eikonal=False, clip_style_latent=None, target_ldks=None):
        ...
        rays = rays.float()
        rgb, features, sdf, mask, xyz, eikonal_term, depth_map, normal_map = self.render_rays(rays, styles=styles, return_eikonal=return_eikonal, clip_style_latent=clip_style_latent, target_ldks=target_ldks)
    ...
    def forward(self, cam_poses, focal, near, far, styles=None, return_eikonal=False, clip_style_latent=None, target_ldks=None):
        rgb, features, sdf, mask, xyz, eikonal_term, depth_map, normal_map = self.render(focal, c2w=cam_poses, near=near, far=far, styles=styles, return_eikonal=return_eikonal, clip_style_latent=clip_style_latent, target_ldks=target_ldks)
