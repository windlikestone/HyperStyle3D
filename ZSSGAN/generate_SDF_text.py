...
# torch.random.manual_seed(330)
torch.random.manual_seed(3009)
# torch.random.manual_seed(111)
# torch.random.manual_seed(528)
...
def generate(opt, g_ema, surface_g_ema, device, mean_latent, surface_mean_latent, text_shape_direction, text_style_direction):
    ...
    if opt.fixed_camera_angles:
        ...
        # locations = torch.tensor([[np.pi * 25 /180, 0]], device=device)
        # locations = torch.tensor([[0, 0]], device=device)
    ...
    for i in tqdm(range(opt.identities)):
        with torch.no_grad():
            ...
            #                        uniform=opt.camera.uniform, azim_range=0,
            #                        elev_range=0, fov_ang=fov,
            ...
            # depth_maps = torch.Tensor(0, 1, opt.size, opt.size)
            ...
            for j in range(0, num_viewdirs, chunk):
                ...
                loc_str = '_azim{}_elev{}'.format(int(curr_locations[0,0] * 180 / np.pi),
                                                  int(curr_locations[0,1] * 180 / np.pi))
                rgb_filename = os.path.join(opt.results_dst_dir,'image','sample_{}{}.png'.format(i, loc_str))
            ...
            utils_a.save_image(rgb_images,
                os.path.join(opt.results_dst_dir, 'images','{}{}.png'.format(str(i).zfill(7),loc_str)),
                nrow=num_viewdirs,
                normalize=True,
                padding=0,
                range=(-1, 1),)
            # utils_a.save_image(rgb_images,
            #     os.path.join(opt.results_dst_dir,'image','sample{}_{}.png'.format(i, loc_str)),
            #     nrow=num_viewdirs,
            #     normalize=True,
            #     padding=0,
            #     range=(-1, 1),)
            ...
            if not opt.no_surface_renderings:
                ...
                for j in range(0, num_viewdirs, surface_chunk):
                    ...
                    for k in range(surface_chunk):
                        ...
                        # depth_mesh = xyz2mesh(xyz[k:k+surface_chunk], background_weights[k:k+surface_chunk],
                        #                           filename=depth_mesh_filename.replace('obj','npz'))
...
if __name__ == "__main__":
    ...
    checkpoints_dir = '/home/chenzhuo/workspace/3DAnimationGAN/ZSSGAN/model/stylesdf_hyper_multi_eyes_model/checkpoint'
    ...
    checkpoint_path_pre = '/home/chenzhuo/workspace/cartoonGAN/ZSSGAN/model/stylesdf_black_man_model/checkpoint/000450.pt'
    # checkpoint_path_pre = '/home/chenzhuo/workspace/cartoonGAN/ZSSGAN/model/pixar_model/checkpoint/000300.pt'
    ...
    # print("checkpoint", checkpoint["g_ema"].keys())
    ...
    interpolate_flag = 1.2
    model_dict = g_ema.state_dict()
    for k, v in model_dict.items():
        ptsObj = re.search( r'hyper_shape_linears', k, re.M|re.I)
        ptsObj_hyper = re.search( r'hyper_linears', k, re.M|re.I)
        ptsObj_0 = re.search( r'pts_linears.4', k, re.M|re.I)
        ptsObj_1 = re.search( r'pts_linears.5', k, re.M|re.I)
        ptsObj_2 = re.search( r'pts_linears.6', k, re.M|re.I)
        ptsObj_3 = re.search( r'pts_linears.7', k, re.M|re.I)
        ptsObj_4 = re.search( r'pts_linears.7', k, re.M|re.I)
        ptsObj_5 = re.search( r'pts_linears.7', k, re.M|re.I)
        ptsObj_rgb = re.search( r'rgb_linears', k, re.M|re.I)
        # ptsObj_linear = re.search( r'pts_linears_0', k, re.M|re.I)
        print(k)
        if ptsObj != None:
            print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        ...
        #     print(interpolate_flag, k)
            ...
            if ptsObj_hyper != None:
                ...
                # if ptsObj_0 != None or ptsObj_1 != None or ptsObj_2 != None or ptsObj_3 != None or ptsObj_rgb != None:
                if True:  
    ...
    with torch.no_grad():
        ...
        # source_img_list = get_dir_img_list(source_img_dir)
        # target_img_list = get_dir_img_list(target_img_dir)
        # img_direction = clip_model.compute_imglist2imglist_direction(source_img_list, target_img_list)
        # img_direction = img_direction.float()
        # print(img_direction)

    generate(opt.inference, g_ema, surface_g_ema, device, mean_latent, surface_mean_latent, text_shape_direction, text_style_direction)
