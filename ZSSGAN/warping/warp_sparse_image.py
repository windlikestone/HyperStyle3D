...
from warping import warp_dense_image 
from warping import interpolate_spline 
...
def _get_grid_locations(image_height, image_width, image_depth):
  ...
  # z_range                = np.linspace(0.88, 1.12, image_depth)
  # y_grid, x_grid, z_grid = np.meshgrid(y_range, x_range, z_range,indexing='ij')
  # grid 3D
  y_range                = np.linspace(0, image_height - 1, image_height)
  x_range                = np.linspace(0, image_width - 1, image_width)
  ...
  return np.stack((y_grid, x_grid, z_grid), -1)
...
def sparse_image_warp(device,
  ...
  batch_size, image_height, image_width, image_depth, _ = list(image.size())
  batch_size = image.shape[0]
  # This generates the dense locations where the interpolant
  # will be evaluated.
  # grid_locations = _get_grid_locations(image_height, image_width)
  grid_locations = _get_grid_locations(image_height, image_width, image_depth)

  # flattened_grid_locations = np.reshape(grid_locations, [image_height * image_width, 2])
  flattened_grid_locations = np.reshape(grid_locations, [image_height * image_width * image_depth, 3])
  ...
  # print("flattened_flows", flattened_flows)
  # dense_flows  = torch.reshape(flattened_flows, (batch_size, image_height, image_width, 2)).to(device)
  dense_flows  = torch.reshape(flattened_flows, (batch_size, image_height, image_width, image_depth, 3)).to(device)
