...
from warping.warp_sparse_image import sparse_image_warp
...
class WarpController(nn.Module):
    ...
    def forward(self, images_rendered: torch.Tensor, landmarks_source: torch.Tensor, landmarks_target: torch.Tensor) -> tuple:
        ...
        # Normalize
        batch_size, image_height, image_width, image_depth, _ = list(images_rendered.size())
        # print(landmarks_source, landmarks_source.shape)
        landmarks_source[..., 0] *= image_height
        landmarks_source[..., 1] *= image_width
        landmarks_source[..., 2] *= image_depth
        landmarks_target[..., 0] *= image_height
        landmarks_target[..., 1] *= image_width
        landmarks_target[..., 2] *= image_depth
        # print(landmarks_source)
        
        landmarks_src = torch.reshape(landmarks_source.detach().clone(), (-1, self.n_ldmark, 3)).to(self.device)
        landmarks_dst = torch.reshape((landmarks_target).detach().clone(), (-1, self.n_ldmark, 3)).to(self.device)
